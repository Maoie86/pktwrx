...
#callslogger = logging.getLogger(__name__)
#callslogger.setLevel(logging.DEBUG)
#formatter_callslogger = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
#callsfile_handler = logging.FileHandler('call_logs.log')  # All logs go here
#callsfile_handler.setLevel(logging.DEBUG)  # Capture all levels in this handler
#callsfile_handler.setFormatter(formatter_callslogger)  # Set the formatter
#callslogger.addHandler(callsfile_handler)
...
async def log_requests(request: Request, call_next):
    ...
    # logger.debug(f"Request from {client_ip} to {method} {endpoint}")
    # logger.debug(str(request))
    # logger.debug(f"Request {method} {endpoint}")
    response = await call_next(request)
    duration = time.time() - start_time
    logger.debug(f"Request {method} {endpoint} took {duration:.4f} seconds")
    return response
