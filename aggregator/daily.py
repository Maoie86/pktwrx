...
def main():
    try:
        ...
        logging.basicConfig(
            filename='/home/mu/aggregator/logs/daily.log',          # Log file name
            level=logging.INFO,          # Log level
            format='%(asctime)s - %(levelname)s - %(message)s'
        )

        logging.info("Daily Aggregator Startup")
        ...
        sql_tx = "SELECT * FROM hourly_data "
        sql_tx = sql_tx + " WHERE yearday = "
        sql_tx = sql_tx + str(yearday)
        ...
        sql_tx = """
            INSERT INTO daily_data (dev_eui, measurement, min, max, ave, sum, count, yearday)
