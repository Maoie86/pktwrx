...
def main():
    try:
        ...
        logging.basicConfig(
            filename='/home/mu/aggregator/logs/monthly.log',          # Log file name
            level=logging.INFO,          # Log level
            format='%(asctime)s - %(levelname)s - %(message)s'
        )

        logging.info("Monthly Aggregator Startup")
        ...
        firstdoy = date(year, month, 1).timetuple().tm_yday
        lastdoy = date(year, month, lastday).timetuple().tm_yday
        fryearday = year * 1000 + firstdoy
        toyearday = year * 1000 + lastdoy
        print(ts, yearmonth, fryearday, toyearday)
        ...
        sql_tx = """
            INSERT INTO monthly_data (dev_eui, measurement, min, max, ave, sum, count, yearmonth)
